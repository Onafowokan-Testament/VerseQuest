...
def add_question(db:Session, chapter:str, question_text:list,options:list, correct_answer:list):
...
def add_user_result(db: Session, user_id:int, question_id:int, user_answers:str, correct_answers:list ):
    """calculates users result"""
    correct_count = sum(1 for users_ans, correct_ans in zip(user_answers, correct_answers) if users_ans == correct_ans)
